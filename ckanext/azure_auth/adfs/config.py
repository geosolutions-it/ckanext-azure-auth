...
class AdfsProviderConfig(BaseProviderConfig):
    ...
    def load_remote_config(self):
        ...
        refresh_time = datetime.now() - timedelta(hours=config[f"{_EXTNAME}.config_reload_interval"])
        if self._config_timestamp is None or self._config_timestamp < refresh_time:
            ...
            if not loaded:
                if self._config_timestamp is None:
                    msg = "Could not load any data from ADFS server. Authentication against ADFS is not possible. "
                    log.critical(msg)
                    raise RuntimeError(msg)
                else:
                    log.warning("Could not load any data from ADFS server. Keeping previous configurations")
    ...
    def _load_openid_config(self):
        ...
        try:
            ...
            response = self.session.get(openid_cfg["jwks_uri"], timeout=TIMEOUT)
            response.raise_for_status()
    ...
    def _load_federation_metadata(self):
        ...
        if config[ATTR_TENANT_ID] == "adfs":
            adfs_config_url = f"{server_url}/FederationMetadata/2007-06/FederationMetadata.xml"
        else:
            adfs_config_url = f"{base_url}/FederationMetadata/2007-06/FederationMetadata.xml"
